...
pluginStatus_t nmsInferenceV2(cudaStream_t stream, const int N, const int perBatchBoxesSize, const int perBatchScoresSize,
    ...
    void* nmsedScores, void* nmsedLdmks, void* workspace, bool isNormalized, bool confSigmoid, bool clipBoxes, int scoreBits)
{
    ...
    status = gatherNMSOutputs(stream, shareLocation, N, numPredsPerClass, numClasses, topK, keepTopK, DT_BBOX,
        DT_SCORE, indices, scores, bboxData, ldmkData, keepCount, nmsedBoxes, nmsedScores,  nmsedLdmks, clipBoxes, scoreShift);
    ASSERT_FAILURE(status == STATUS_SUCCESS);
